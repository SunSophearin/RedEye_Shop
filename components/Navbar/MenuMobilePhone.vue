...
const { data: categories } = await useAsyncData<CategoryDto[]>('menubar', () => fectMenubar())

// Transform categories → navigation menu items
const menubar = computed(() =>
  (categories.value || []).map((category:any) => ({
    label: category.name,
    class:"text-black dark:text-[#FFFFFF] text-3xl Oswald uppercase py-3 tracking-wide",
    children: (category.categoryTypes || []).map((type:any) => ({
      label: type.name,
      class:"text-[#212121] dark:text-[#E0E0E0] Oswald uppercase tracking-wide",
